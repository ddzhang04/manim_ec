...
class SlopedSRAS(Scene):
    """Upward-sloping SRAS: P = Pᵉ + slope·(Y − Ȳ).

    Demand shock with long-run adjustment.
    """
    def construct(self):
        diagram = ADASDiagram(
            m=20, v=1, sras_price=4, sras_slope=0.5, lras_y=5,
        )
        self.play(Create(diagram))
        self.wait()

        label = Text("Sloped SRAS — demand shock: M ↑",
